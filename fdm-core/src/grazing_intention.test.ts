...
describe("Grazing Intention", () => {
    ...
    describe("setGrazingIntention", () => {
        it("should add a new grazing intention", async () => {
            ...
            await setGrazingIntention(fdm, principal_id, b_id_farm, b_grazing_intention_year, intention)

            const result = await getGrazingIntention(fdm, principal_id, b_id_farm, b_grazing_intention_year)
            expect(result).toBe(intention)
        })

        it("should update an existing grazing intention", async () => {
            const b_grazing_intention_year = 2026
            await setGrazingIntention(fdm, principal_id, b_id_farm, b_grazing_intention_year, true)
            await setGrazingIntention(fdm, principal_id, b_id_farm, b_grazing_intention_year, false)
        ...
        it("should throw an error if principal does not have permission", async () => {
            ...
            await expect(
                setGrazingIntention(fdm, invalidPrincipal, b_id_farm, 2027, true),
    ...
    describe("getGrazingIntentions", () => {
        it("should return all grazing intentions for a farm", async () => {
            ...
            const intentions = await getGrazingIntentions(fdm, principal_id, b_id_farm)
        ...
        it("should return an empty array if no intentions are set", async () => {
            const new_b_id_farm = await addFarm(fdm, principal_id, "Farm Without Intentions", undefined, undefined, undefined)
            const intentions = await getGrazingIntentions(fdm, principal_id, new_b_id_farm)
    ...
    describe("removeGrazingIntention", () => {
        it("should remove an existing grazing intention", async () => {
            ...
            let result = await getGrazingIntention(fdm, principal_id, b_id_farm, b_grazing_intention_year)
            expect(result).toBe(true)

            await removeGrazingIntention(fdm, principal_id, b_id_farm, b_grazing_intention_year)

            result = await getGrazingIntention(fdm, principal_id, b_id_farm, b_grazing_intention_year)
            expect(result).toBe(false)
        })
        ...
        it("should not throw an error when removing a non-existent intention", async () => {
            ...
            await expect(
                removeGrazingIntention(fdm, principal_id, b_id_farm, b_grazing_intention_year),
        ...
        it("should throw an error if principal does not have permission", async () => {
            ...
            await expect(
                removeGrazingIntention(fdm, invalidPrincipal, b_id_farm, b_grazing_intention_year),
            ).rejects.toThrowError("Principal does not have permission to perform this action")
        })
    })
})
