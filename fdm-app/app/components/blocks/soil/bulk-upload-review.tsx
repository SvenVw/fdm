  ...
  ({
    ...
    if (isMatched) {
      ...
      if (isAutomatic && matchReason) {
        if (matchReason === "geometry") tooltipText = "Automatisch gekoppeld op basis van geometrie"
        else if (matchReason === "name") tooltipText = "Automatisch gekoppeld op basis van naam"
        else if (matchReason === "both")
          tooltipText = "Automatisch gekoppeld op basis van geometrie en naam"
      }
...
export function BulkSoilAnalysisReview({
  ...
  const [dates, setDates] = useState<Record<string, string>>(
    Object.fromEntries(
      analyses.map((a) => {
        const date = a.b_sampling_date ? new Date(a.b_sampling_date) : null
        const validDateStr = date && isValid(date) ? format(date, "yyyy-MM-dd") : ""
        return [a.id, validDateStr]
      }),
    ),
  )

  const handleFieldChange = useCallback((analysisId: string, fieldId: string) => {
  ...
  const sourceLabelMap = useMemo(() => {
    ...
    return Object.fromEntries(
      sourceParam?.options?.map((x: { value: string; label: string }) => [x.value, x.label]) || [],
    )
  }, [soilParameterDescription])
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          const sourceLabel =
            sourceLabelMap[row.original.a_source] || row.original.a_source || "Onbekend"
        ...
        cell: ({ row }) => (
          <div className="flex flex-wrap gap-1">
            ...
            {row.original.a_p_cc != null && (
              <Badge variant="secondary">P-CaCl₂: {row.original.a_p_cc}</Badge>
            )}
            {row.original.a_nmin_cc != null && (
              <Badge variant="secondary">Nmin: {row.original.a_nmin_cc}</Badge>
            )}
            {row.original.a_nh4_cc != null && (
              <Badge variant="secondary">NH₄: {row.original.a_nh4_cc}</Badge>
            )}
            {row.original.a_no3_cc != null && (
              <Badge variant="secondary">NO₃: {row.original.a_no3_cc}</Badge>
            )}
          </div>
        ),
      },
        ...
        cell: ({ row, table }) => {
          ...
          return (
            <MatchCell
              ...
              isDateValid={isValidDate(meta.dates[row.original.id])}
    ...
    <TooltipProvider>
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            Controleer de gegevens uit de pdf's en koppel ze aan het juiste perceel. Analyses met
            ontbrekende datum of zonder gekoppeld perceel worden overgeslagen.
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardFooter className="flex justify-between border-t p-6">
          ...
          <Button onClick={handleSave} disabled={validMatches.length === 0}>
