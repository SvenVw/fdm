...
export const mappers = {
  ...
  mapByDate: createMapper((application) => application.p_app_date.getTime().toString()),
  ...
  mapBySimilarAndOrder: (record: FertAppRecord, applications: ApplicationExtended[]) => {
...
function groupAndOrderFertApps(
  ...
  const entries = keysToSortById.includes(mapper)
    ? // There is an ordering imposed by the mapper which is the same as the ordering of the keys
      unsortedEntries
        .map(([key, value]) => [Number.parseFloat(key), value] as [number, FertAppRecordItem])
        .sort((a, b) => a[0] - b[0])
    : // There is no inherent ordering, sort by date and field name of first application
      unsortedEntries.sort(
        (a, b) =>
          ...
          ) || compareStrings(a[1].applications[0].b_name, b[1].applications[0].b_name),
      )
...
export function DataTable({
  ...
  const records = useMemo(
    () => groupAndOrderFertApps(fertilizerApplications, "mapBySimilarAndOrder"),
    [fertilizerApplications],
  )
  ...
  const columnVisibility = useMemo(
    () => ({
      ...
      "applications.length": records.some(
        (record) =>
          record.applications.length > new Set(record.applications.map((app) => app.b_id)).size,
      ),
      // Only show the modify actions column if any of the applications is modifiable
      modify: fertilizerApplications.some((apps) => apps.some((app) => app.canModify)),
    }),
