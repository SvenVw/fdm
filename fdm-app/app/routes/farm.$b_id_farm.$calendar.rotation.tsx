...
export async function action({ params, request }: Route.ActionArgs) {
  try {
    ...
    const cultivationUpdates = await extractFormValuesFromRequest(request, RotationTableFormSchema)

    if (
      Object.keys(cultivationUpdates).length > 0 &&
      Object.values(cultivationUpdates).some((val) => typeof val !== "undefined")
    ) {
      // Perform the cultivation updates
      const fields = (await getFields(fdm, session.principal_id, b_id_farm, timeframe)).filter(
        (field) => fieldIds.has(field.b_id),
      )
      await Promise.all(
        fields.map(async (field) => {
          const cultivations = (
            await getCultivations(fdm, session.principal_id, field.b_id, timeframe)
          ).filter((cultivation) => cultivationIds.has(cultivation.b_lu_catalogue))
