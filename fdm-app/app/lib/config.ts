...
import type { RuntimeConfig } from "~/types/vite-env.d" // Import the new RuntimeConfig
...
const getConfigValue = (key: keyof RuntimeConfig, defaultValue?: RuntimeConfig[keyof RuntimeConfig]) => {
...
const sentryConfig =
    sentryDsn && sentryOrg && sentryProject
        ? {
              ...
              trace_sample_rate:
                  Number(getConfigValue("PUBLIC_SENTRY_TRACE_SAMPLE_RATE", 1)),
              replay_sample_rate:
                  Number(getConfigValue("PUBLIC_SENTRY_REPLAY_SAMPLE_RATE", 0)),
              replay_sample_rate_on_error:
                  Number(
                      getConfigValue("PUBLIC_SENTRY_REPLAY_SAMPLE_RATE_ON_ERROR", 1)
                  ),
              profile_sample_rate:
                  Number(getConfigValue("PUBLIC_SENTRY_PROFILE_SAMPLE_RATE", 1)),
