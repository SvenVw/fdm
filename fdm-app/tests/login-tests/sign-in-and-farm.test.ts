...
test("User can create a farm business and select parcels", async ({
    page,
    context,
}) => {
    ...
    if (
        !currentCookies.find((c) => c.name === "better-auth.session_token") ||
    ...
    await expect(businessNameBox, "There is no business name box").toBeVisible()
