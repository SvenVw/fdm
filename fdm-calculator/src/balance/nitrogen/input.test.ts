...
const mockedGetFields = fdmCoreMocks.getFields as vi.MockedFunction<typeof fdmCoreMocks.getFields>
const mockedGetCultivations = fdmCoreMocks.getCultivations as vi.MockedFunction<typeof fdmCoreMocks.getCultivations>
const mockedGetHarvests = fdmCoreMocks.getHarvests as vi.MockedFunction<typeof fdmCoreMocks.getHarvests>
const mockedGetSoilAnalyses = fdmCoreMocks.getSoilAnalyses as vi.MockedFunction<typeof fdmCoreMocks.getSoilAnalyses>
const mockedGetFertilizerApplications = fdmCoreMocks.getFertilizerApplications as vi.MockedFunction<typeof fdmCoreMocks.getFertilizerApplications>
const mockedGetFertilizers = fdmCoreMocks.getFertilizers as vi.MockedFunction<typeof fdmCoreMocks.getFertilizers>
const mockedGetCultivationsFromCatalogue = fdmCoreMocks.getCultivationsFromCatalogue as vi.MockedFunction<typeof fdmCoreMocks.getCultivationsFromCatalogue>
...
describe("collectInputForNitrogenBalance", () => {
    ...
    it("should collect input successfully when all data is available", async () => {
        ...
        const mockFieldsData: Pick<Field, "b_id" | "b_centroid" | "b_area">[] = [
            { b_id: "field-1", b_centroid: { type: "Point", coordinates: [0,0] }, b_area: 10 },
            { b_id: "field-2", b_centroid: { type: "Point", coordinates: [1,1] }, b_area: 20 },
        ]
        const mockCultivationsData: Pick<Cultivation, "b_lu" | "b_lu_catalogue" | "m_cropresidue">[] = [
            { b_lu: "cult-1", b_lu_catalogue: "cat-cult-1", m_cropresidue: 0.5 },
        ]
        const mockHarvestsData: Harvest[] = [
            { b_harvesting_id: "harvest-1", b_lu: "cult-1", b_lu_harvest_date: new Date(), b_lu_yield: 1000, b_id_field: "field-1", b_id_farm: b_id_farm, b_principal_id_field: principal_id, b_principal_id_farm: principal_id },
        ]
        const mockSoilAnalysesData: Pick<SoilAnalysis, "a_id" | "a_c_of" | "a_cn_fr" | "a_density_sa" | "a_n_rt" | "b_soiltype_agr">[] = [
            { a_id: "sa-1", a_c_of: 25, a_cn_fr: 10, a_density_sa: 1.5, a_n_rt: 100, b_soiltype_agr: "SAND" },
        ]
        const mockFertilizerApplicationsData: FertilizerApplication[] = [
            { b_fertilizing_id: "fa-1", p_id_fertilizer: "fert-1", p_amount: 100, p_id_field: "field-1", p_id_farm: b_id_farm, p_date_applying: new Date(), p_principal_id_field: principal_id, p_principal_id_farm: principal_id },
        ]
        const mockFertilizerDetailsData: FertilizerDetail[] = [
            { p_id_catalogue: "fert-cat-1", p_n_rt: 5, p_type_manure: "cattle", p_type_mineral: null, p_type_compost: null },
        ]
        const mockCultivationDetailsData: CultivationDetail[] = [
            { b_lu_catalogue: "cat-cult-1", b_lu_croprotation: "maize", b_lu_yield: 5000, b_lu_hi: 0.45, b_lu_n_harvestable: 1.2, b_lu_n_residue: 0.8, b_n_fixation: 0 },
        ]
        ...
        mockedGetCultivationsFromCatalogue.mockResolvedValue(mockCultivationDetailsData)
        ...
        // Verify calls
        expect(mockedGetFields).toHaveBeenCalledWith(mockFdm, principal_id, b_id_farm, timeframe)
        for (const field of mockFieldsData) {
            expect(mockedGetCultivations).toHaveBeenCalledWith(mockFdm, principal_id, field.b_id, timeframe)
            // For each cultivation, getHarvests is called
            for (const cultivation of mockCultivationsData) {
                 expect(mockedGetHarvests).toHaveBeenCalledWith(mockFdm, principal_id, cultivation.b_lu, timeframe)
            }
            expect(mockedGetSoilAnalyses).toHaveBeenCalledWith(mockFdm, principal_id, field.b_id, timeframe)
            expect(mockedGetFertilizerApplications).toHaveBeenCalledWith(mockFdm, principal_id, field.b_id, timeframe)
        }
        expect(mockedGetFertilizers).toHaveBeenCalledWith(mockFdm, principal_id, b_id_farm)
        expect(mockedGetCultivationsFromCatalogue).toHaveBeenCalledWith(mockFdm, principal_id, b_id_farm)
    })
