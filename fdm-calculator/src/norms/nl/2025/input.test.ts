...
import type { Field, Cultivation, SoilAnalysis, FdmType, Timeframe } from "@svenvw/fdm-core";
...
describe("collectNL2025InputForNorms", () => {
    it("should collect input correctly", async () => {
        ...
        const mockTimeframe = { start: new Date(), end: new Date() } as Timeframe;

        const mockField = { b_id: mockFieldId, b_id_farm: "farm-1", b_centroid: { type: "Point", coordinates: [5.0, 52.0] } } as Field;
        ...
        vi.mocked(fdmCore.getCurrentSoilData).mockResolvedValue(mockSoilAnalysis as unknown as SoilAnalysis[]);
        ...
        expect(fdmCore.getCultivations).toHaveBeenCalledWith(mockFdm, mockPrincipalId, mockFieldId, mockTimeframe);
        expect(fdmCore.getCurrentSoilData).toHaveBeenCalledWith(mockFdm, mockPrincipalId, mockFieldId, mockTimeframe);
    });
});
